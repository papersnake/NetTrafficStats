...
public final class ofirewallApi {
	...
	// Preferences
	public static final String PREFS_NAME 		= "OFireWallPrefs";
	...
	// Messages
	public static final String STATUS_CHANGED_MSG 	= "com.orderlong.netTrafficStats.intent.action.STATUS_CHANGED";
	public static final String TOGGLE_REQUEST_MSG	= "com.orderlong.netTrafficStats.intent.action.TOGGLE_REQUEST";
	public static final String STATUS_EXTRA			= "com.orderlong.netTrafficStats.intent.extra.STATUS";
	...
	private static String scriptHeader(Context ctx) {
		...
		return "" +
			...
			"	if ! $ECHO 1 | $GREP -q 1 >/dev/null 2>/dev/null ; then\n" +
			"		$ECHO The grep command is required. ofirewall will not work.\n" +
			"		exit 1\n" +
			"	fi\n" +
			"fi\n" +
			"# Try to find iptables\n" +	
			"if /system/bin/iptables --version >/dev/null 2>/dev/null ; then\n" +
			"	IPTABLES=/system/bin/iptables\n" +
			"elif /system/xbin/iptables --version >/dev/null 2>/dev/null ; then\n"+
			"	IPTABLES=/system/xbin/iptables\n" +
			"elif " + myiptables + " --version >/dev/null 2>/dev/null ; then\n" +
			"	IPTABLES="+myiptables+"\n" +
			"fi\n" +
			"";
	}
	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors) {
		...
		try {
			...
			script.append("" +
				...
				"# Create the ofirewall chains if necessary\n" +
				"$IPTABLES -L ofirewall >/dev/null 2>/dev/null || $IPTABLES --new ofirewall || exit 2\n" +
				"$IPTABLES -L ofirewall-3g >/dev/null 2>/dev/null || $IPTABLES --new ofirewall-3g || exit 3\n" +
				"$IPTABLES -L ofirewall-wifi >/dev/null 2>/dev/null || $IPTABLES --new ofirewall-wifi || exit 4\n" +
				"$IPTABLES -L ofirewall-reject >/dev/null 2>/dev/null || $IPTABLES --new ofirewall-reject || exit 5\n" +
				"# Add ofirewall chain to OUTPUT chain if necessary\n" +
				"$IPTABLES -L OUTPUT | $GREP -q ofirewall || $IPTABLES -A OUTPUT -j ofirewall || exit 6\n" +
				"# Flush existing rules\n" +
				"$IPTABLES -F ofirewall || exit 7\n" +
				"$IPTABLES -F ofirewall-3g || exit 8\n" +
				"$IPTABLES -F ofirewall-wifi || exit 9\n" +
				"$IPTABLES -F ofirewall-reject || exit 10\n" +
			"");
			// Check if logging is enabled
			if (logenabled) {
				script.append("" +
					"# Create the log and reject rules (ignore errors on the LOG target just in case it is not available)\n" +
					"$IPTABLES -A ofirewall-reject -j LOG --log-prefix \"[ofirewall] \" --log-uid\n" +
					"$IPTABLES -A ofirewall-reject -j REJECT || exit 11\n" +
				"");
			} else {
				script.append("" +
			...
			if (whitelist && logenabled) {
				...
				script.append("$IPTABLES -A ofirewall -p udp --dport 53 -j RETURN\n");
			}
			script.append("# Main rules (per interface)\n");
			for (final String itf : ITFS_3G) {
				script.append("$IPTABLES -A ofirewall -o ").append(itf).append(" -j ofirewall-3g || exit\n");
			}
			for (final String itf : ITFS_WIFI) {
				script.append("$IPTABLES -A ofirewall -o ").append(itf).append(" -j ofirewall-wifi || exit\n");
			}
			
			script.append("# Filtering rules\n");
			final String targetRule = (whitelist ? "RETURN" : "ofirewall-reject");
			...
			if (whitelist && !any_wifi) {
				...
				if (uid != -1) {
					...
					script.append("$IPTABLES -A ofirewall-wifi -m owner --uid-owner ").append(uid).append(" -j RETURN || exit\n");
				}
			...
			if (any_3g) {
				if (blacklist) {
					...
					script.append("$IPTABLES -A ofirewall-3g -j ").append(targetRule).append(" || exit\n");
				}
			} else {
				/* release/block individual applications on this interface */
				for (final Integer uid : uids3g) {
					if (uid >= 0) script.append("$IPTABLES -A ofirewall-3g -m owner --uid-owner ").append(uid).append(" -j ").append(targetRule).append(" || exit\n");
				}
			}
			if (any_wifi) {
				if (blacklist) {
					/* block any application on this interface */
					script.append("$IPTABLES -A ofirewall-wifi -j ").append(targetRule).append(" || exit\n");
				}
			} else {
				/* release/block individual applications on this interface */
				for (final Integer uid : uidsWifi) {
					if (uid >= 0) script.append("$IPTABLES -A ofirewall-wifi -m owner --uid-owner ").append(uid).append(" -j ").append(targetRule).append(" || exit\n");
				}
			}
			if (whitelist) {
				if (!any_3g) {
					if (uids3g.indexOf(SPECIAL_UID_KERNEL) >= 0) {
						...
						script.append("$IPTABLES -A ofirewall-3g -m owner --uid-owner 0:999999999 -j ofirewall-reject || exit\n");
					} else {
						script.append("$IPTABLES -A ofirewall-3g -j ofirewall-reject || exit\n");
					}
				}
				if (!any_wifi) {
					if (uidsWifi.indexOf(SPECIAL_UID_KERNEL) >= 0) {
						script.append("# hack to allow kernel packets on white-list\n");
						script.append("$IPTABLES -A ofirewall-wifi -m owner --uid-owner 0:999999999 -j ofirewall-reject || exit\n");
					} else {
						script.append("$IPTABLES -A ofirewall-wifi -j ofirewall-reject || exit\n");
					}
				}
			} else {
				if (uids3g.indexOf(SPECIAL_UID_KERNEL) >= 0) {
					script.append("# hack to BLOCK kernel packets on black-list\n");
					script.append("$IPTABLES -A ofirewall-3g -m owner --uid-owner 0:999999999 -j RETURN || exit\n");
					script.append("$IPTABLES -A ofirewall-3g -j ofirewall-reject || exit\n");
				}
				if (uidsWifi.indexOf(SPECIAL_UID_KERNEL) >= 0) {
					script.append("# hack to BLOCK kernel packets on black-list\n");
					script.append("$IPTABLES -A ofirewall-wifi -m owner --uid-owner 0:999999999 -j RETURN || exit\n");
					script.append("$IPTABLES -A ofirewall-wifi -j ofirewall-reject || exit\n");
				}
			}
    ...
    */
	...
	public static void saveRules(Context ctx)
	{
		...
		final TrafficApp app=(TrafficApp)ctx.getApplicationContext();
		final List<Integer> saveduids_wifi=app.getSavedUids_wifi();
		for(Integer i:saveduids_wifi)
		{
			if(newuids_wifi.length() != 0) newuids_wifi.append('|');
			newuids_wifi.append(i);
		}
		
		final List<Integer> saveduids_3g=app.getSavedUids_3g();
		for(Integer i:saveduids_3g)
		{
			if(newuids_3g.length() !=0) newuids_3g.append('|');
			newuids_3g.append(i);
		}
	...
	public static boolean purgeIptables(Context ctx, boolean showErrors) {
		...
		try {
			...
			int code = runScriptAsRoot(ctx, scriptHeader(ctx) +
					"$IPTABLES -F ofirewall\n" +
					"$IPTABLES -F ofirewall-reject\n" +
					"$IPTABLES -F ofirewall-3g\n" +
					"$IPTABLES -F ofirewall-wifi\n", res);
			if (code == -1) {
	...
	public static void showLog(Context ctx) {
		try {
			...
			while ((line = r.readLine()) != null) {
				if (line.indexOf("[ofirewall]") == -1) continue;
				appid = unknownUID;
	...
	public static boolean hasRootAccess(Context ctx, boolean showErrors) {
		...
		if (showErrors) {
			...
			alert(ctx, "Could not acquire root access.\n" +
				"You need a rooted phone to run ofirewall.\n\n" +
				"If this phone is already rooted, please make sure ofirewall has enough permissions to execute the \"su\" command.\n" +
				"Error message: " + res.toString());*/
			alert(ctx,ctx.getResources().getString(R.string.notrooted));
		}
		return false;
	}
    /**
	...
	public static void setEnabled(Context ctx, boolean enabled) {
		...
		/* notify */
		final Intent message = new Intent(ofirewallApi.STATUS_CHANGED_MSG);
        message.putExtra(ofirewallApi.STATUS_EXTRA, enabled);
	...
	public static void applicationRemoved(Context ctx, int uid) {
		...
		if (savedUids_wifi.length() > 0) {
			...
			while (tok.hasMoreTokens()) {
				...
				if (uid_str.equals(token)) {
					Log.d("ofirewall", "Removing UID " + token + " from the wi-fi list (package removed)!");
					changed = true;
				} else {
		...
		if (savedUids_3g.length() > 0) {
			...
			while (tok.hasMoreTokens()) {
				final String token = tok.nextToken();
				if (uid_str.equals(token)) {
					Log.d("ofirewall", "Removing UID " + token + " from the 3G list (package removed)!");
					changed = true;
				} else {
					if (newuids.length() > 0) newuids.append('|');
					newuids.append(token);
